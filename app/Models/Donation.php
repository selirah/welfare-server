...
class Donation extends Model
{
    ...
    public function _gets($institutionId, $month, $year)
    {
        if (!empty($month) && !empty($year)) {
            return $this->_connectTable()
                ->select([$this->table . '.*', 'donation_settings.type'])
                ->where($this->table . '.institution_id', '=', $institutionId)
                ->where('month', '=', $month)
                ->where('year', '=', $year)
                ->leftJoin('donation_settings', 'donation_settings.id', '=', $this->table . '.type_id')
    ...
    public function _getOfficeDonations($institutionId, $month, $year)
    {
        if (!empty($month) && !empty($year)) {
            return $this->_connectTable()
                ->select([$this->table . '.*', 'donation_settings.type'])
    ...
    public function _getMembersDonations($institutionId, $month, $year)
    {
        if (!empty($month) && !empty($year)) {
            return $this->_connectTable()
                ->select([$this->table . '.*', 'donation_settings.type', 'members.member_name', 'members.member_phone'])
                ->where($this->table . '.institution_id', '=', $institutionId)
                ->where('month', '=', $month)
                ->where('year', '=', $year)
    ...
    public function _getMemberDonations($institutionId, $staffId, $month, $year)
    {
        if (!empty($month) && !empty($year)) {
            return $this->_connectTable()
                ->select([$this->table . '.*', 'donation_settings.type', 'members.member_name', 'members.member_phone'])
