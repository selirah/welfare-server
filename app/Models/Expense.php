...
class Expense extends Model
{
    ...
    public function _gets($institutionId, $month, $year)
    {
        if (!empty($month) && !empty($year)) {
            return $this->_connectTable()
                ->select([$this->table . '.*', 'expense_settings.type'])
                ->where($this->table . '.institution_id', '=', $institutionId)
                ->where('month', '=', $month)
                ->where('year', '=', $year)
                ->leftJoin('expense_settings', 'expense_settings.id', '=', $this->table . '.type_id')
    ...
    public function _getOfficeExpenses($institutionId, $month, $year)
    {
        if (!empty($month) && !empty($year)) {
            return $this->_connectTable()
                ->select([$this->table . '.*', 'expense_settings.type'])
    ...
    public function _getMembersExpenses($institutionId, $month, $year)
    {
        if (!empty($month) && !empty($year)) {
            return $this->_connectTable()
                ->select([$this->table . '.*', 'expense_settings.type', 'members.member_name', 'members.member_phone'])
                ->where($this->table . '.institution_id', '=', $institutionId)
                ->where('month', '=', $month)
                ->where('year', '=', $year)
    ...
    public function _getMemberExpenses($institutionId, $staffId, $month, $year)
    {
        if (!empty($month) && !empty($year)) {
            return $this->_connectTable()
                ->select([$this->table . '.*', 'expense_settings.type', 'members.member_name', 'members.member_phone'])
